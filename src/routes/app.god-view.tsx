...
function GodViewPage() {
  ...
  const bottlenecks = [
    { label: "Delayed trips", value: delayed, tone: "warning" as const, to: "/app/trips" },
    { label: "Maintenance backlog", value: WORK_ORDERS.filter((w) => w.status !== "Completed").length, tone: "critical" as const, to: "/app/engineering" },
    { label: "Pending approvals", value: EXPENSES.filter((e) => e.status === "Pending").length, tone: "info" as const, to: "/app/accounts" },
    { label: "Driver shortages", value: DRIVERS.filter((d) => d.status === "Available").length < 8 ? 1 : 0, tone: "warning" as const, to: "/app/drivers" },
    { label: "Low inventory", value: INVENTORY.filter((i) => i.status !== "In Stock").length, tone: "warning" as const, to: "/app/inventory" },
  ];
