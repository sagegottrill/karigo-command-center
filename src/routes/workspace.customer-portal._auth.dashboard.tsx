...
function SisterCompanyDashboard() {
  ...
  const columns: Column<Trip>[] = [
    ...
    { key: "customerConsignee", header: "Consignee", cell: (r) => <span className="text-xs">{r.customerConsignee || "â€”"}</span> },
    { key: "pickup", header: "Pickup", cell: (r) => <span className="text-xs">{r.loadingSite && r.loadingSite.length > 1 ? `${r.loadingSite.length} Sites` : (r.pickup || "â€”")}</span> },
    { key: "dropoff", header: "Destination", cell: (r) => <span className="text-xs">{r.dropoff}</span> },
    { key: "tailType", header: "Tail Type", cell: (r) => <span className="text-xs">{r.tailType || "â€”"}</span> },
