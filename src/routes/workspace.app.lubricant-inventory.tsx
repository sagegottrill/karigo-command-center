...
export const Route = createFileRoute("/workspace/app/lubricant-inventory")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Diesel and gas in the tank, and every restock that put it there." },
    ],
  }),
...
function LubricantInventoryPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [next, rows] = await Promise.all([lubricantService.overview(), lubricantService.restocks()]);
      setOverview(next);
      setRestocks(rows);
    } catch (err) {
      // Keep whatever is on screen — a poll that fails must not blank the tank.
      if (loading) toast.error(err instanceof Error ? err.message : "Failed to load the lubricant inventory.");
    } finally {
      setLoading(false);
    }
  }, [loading]);
      ...
      <div className="flex flex-col gap-4 rounded-[10px] border border-[#E2E5E9] bg-white p-4 shadow-[0px_4px_16px_rgba(12,12,13,0.05)] md:p-5">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[640px]">
            <div className={cn("items-center gap-x-3 border-b border-[#E2E5E9] py-[15px]", RESTOCK_GRID)}>
              <span className="text-[16px] font-semibold tracking-[0.4px] text-[#1B2432]">Restock ID</span>
              <span className="text-[16px] font-semibold tracking-[0.4px] text-[#1B2432]">Date</span>
              ...
              <span className="text-[16px] font-semibold tracking-[0.4px] text-[#1B2432]">Logged by</span>
            </div>
            ...
            {slice.length === 0 ? (
              ...
              slice.map((row) => (
                <div
                  key={row.id}
                  className={cn("items-center gap-x-3 border-b border-[#E2E5E9] py-2.5 last:border-b-0", RESTOCK_GRID)}
                >
                  ...
                  <span className="truncate text-[14px] tracking-[0.4px] text-[#5C6470]">{row.fuelType}</span>
                  <span className="flex items-baseline gap-1.5 truncate">
                    <span className="text-[14px] font-medium tabular-nums text-[#141A1F]">{formatQuantity(row.quantity)}</span>
                    <span className="text-[10.5px] uppercase tracking-[0.4px] text-[#627084]">
                      {lubricantUnit(row.fuelType)}
                    </span>
                  </span>
                  <span className="truncate text-[14px] tracking-[0.4px] text-[#5C6470]">{row.loggedBy}</span>
                </div>
              ))
            )}
          </div>
        ...
        <LubricantTableFooter
          ...
          onExport={() =>
            exportCsv(
              ...
              filtered.map((r) => [r.reference, csvStamp(r.createdAt), r.fuelType, r.quantity, lubricantUnit(r.fuelType), r.loggedBy]),
            )
          }
        />
      </div>
