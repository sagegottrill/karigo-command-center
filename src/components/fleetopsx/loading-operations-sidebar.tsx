 ...
 */
const LOADING_NAV: LoadingNavItem[] = [
...
/** Loading shell — never Transport Manager, Fleet Ops, Tracking or Security. */
export function shouldUseLoadingShell(roles: string[]) {
  ...
  if (roles.some((r) => /tracking/i.test(r))) return false;
  return roles.some((r) => /loading/i.test(r));
}

export function LoadingOperationsSidebar({
  collapsed,
  onToggle,
}: {
  collapsed: boolean;
...
/** Loading department mobile bottom tab bar. */
